...
##'     historical trajectories, and one for the partially observed current
##'     season, containing NA's for all future or missing ' data points to
##'     forecast or infer; currently only supports NA's for future ' data points,
##'     not mixed in with past data.
##' @param baseline a "baseline level" for this dataset; roughly speaking, data
##'   below this level does not grow like an epidemic.
...
##'   length(new.dat)
##' @param n.sim number of (weighted) simulated curves to produce. Defaults to 1000.
##'
##' @return a list with two components:
##'
#' \code{ys}: a numeric matrix; each column is a different possible trajectory
##' for the current season, with NA's in new.dat filled in with random draws
##' from the forecasted distribution, and non-NA's (observed data) filled in
##' with an imagined resampling of noise based on the model.
##'
##' \code{weights}: a numeric vector; assigns a weight to each column of
##' \code{ys}; generated by importance sampling.
##'
##' @author Logan C. Brooks, David C. Farrow, Sangwon Hyun, Ryan J. Tibshirani, Roni Rosenfeld
##'
##' @export
